...
cd "$datafolder/Consumption data"
...
use s11a, clear
append using s11a1 s11b s11c s11d
...
ren regioncode id1
ren districtcode id2
ren eacode id3
ren hhid id4
keep id1 id2 id3 id4 FPrimary food_id itname s11a_bi s11ab s11a_ci s11ac s11a_di ///
...
sort FPrimary food_id item_id fuel_id
bys diff: distinct FPrimary //639 distinct HH; 8347 obs with differing totals
...
bys FPrimary: egen food_exp = total(s11ac)
la var food_exp "Total monthly expenditure on food"
bys FPrimary: egen gifts_given = total(s11ae)
la var gifts_given "Total monthly expenditure on giving gifts"
bys FPrimary: egen own_produce = total(s11ab)
...
foreach var of varlist s11ba s11bb s11bc s11bd s11c s11db_new {
	...
	bys FPrimary : egen `name'_exp = total(`var')
...
bys FPrimary: egen avg_monthly_exp = total(total_expenditure)
...
drop uid qbi_dummy qci_dummy qei_dummy count_quantity_med_nat_Qbi count_quantity_med_nat_Qci count_quantity_med_nat_Qei

order id1 id2 id3 id4 FPrimary itname food_id unit Q* s11a* s11b* total_exp_s11b itemname item_id s11c* fuel_id months_used s11d* food_exp gifts_given ///
...
save "$resultfolder/consumption_expenditure.dta", replace 

keep id1 id2 id3 id4 FPrimary total_exp_s11b food_exp gifts_given own_produce avg_monthly_exp *exp
drop if id4==.
drop if mi(avg_monthly_exp)

save "$resultfolder/aggregated_expenditure.dta", replace
************************************************************************************************************************************
...
global datafolder "/Users/nanayaw/Desktop/Wave 1"	
global resultfolder "/Users/nanayaw/Desktop/Wave 1/Data results"

use "$datafolder/S1FI.dta", clear
keep FPrimary s1fi_13i-s1fi_21ii
...
drop *itemized*
save "$resultfolder/education", replace


use "$datafolder/S1FIII.dta", clear
keep FPrimary *67*
egen training_exp = rowtotal(s1fiii_67i s1fiii_67ii)
collapse (sum)training_exp, by(FPrimary)

merge 1:1 FPrimary using "$resultfolder/education", nogen
...
use "$datafolder/S6A.dta", clear
keep FPrimary *7* 
egen insurance_premium = rowtotal(s6a_a7ai s6a_a7aii)
collapse (sum)insurance_premium, by(FPrimary)
save "$resultfolder/health", replace


use "$datafolder/S6C", clear
keep FPrimary s6c_10i s6c_10ii s6c_111i-s6c_117ii
...
egen itemized_total = rowtotal(s6c_111-s6c_117)
ren s6c_10 non_itemized_total 
collapse (sum)itemized_total (sum) non_itemized_total, by(FPrimary)
replace itemized_total = 0 if itemized_total!=0 & non_itemized_total!=0	//people might have had a better idea of an overall number so I am discarding even the few items they entered
gen vaccine_total= itemized_total + non_itemized_total
drop *itemized*

merge 1:1 FPrimary using "$resultfolder/health", nogen
save "$resultfolder/health", replace


use "$datafolder/S6F.dta", clear 
keep FPrimary s6f_15i-s6f_16ii s6f_22i s6f_22ii s6f_24i-s6f_25ii 
...
collapse (sum)health_expenses, by(FPrimary)
...
*DWELLING (MONTHLY)
use "$datafolder/S12AI.dta", clear 
keep FPrimary *2i* *4i* *7i* *8i* *18i* *19i* *23i* *26i* *29i*
...
collapse (sum)dwelling_expenses, by(FPrimary)
	la var dwelling_expenses "Dwelling exp monthly"
save "$resultfolder/dwelling", replace


use "$resultfolder/aggregated_expenditure.dta", clear
merge 1:1 FPrimary using "$resultfolder/education", nogen
merge 1:1 FPrimary using "$resultfolder/health", nogen
merge 1:1 FPrimary using "$resultfolder/dwelling", nogen
...
save "$resultfolder/aggregated_expenditure", replace
